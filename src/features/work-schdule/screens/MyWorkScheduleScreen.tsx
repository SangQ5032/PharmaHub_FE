...
const MyWorkScheduleScreen = () => {
  const { data, isLoading, error, refetch, isRefetching } = useMyWorkSchedule();
      ...
      >
        <View style={styles.cardHeader}>
          <View style={styles.iconContainer}>
            <Icon
              name={isToday ? 'calendar-check' : 'calendar'}
              size={24}
              color="#4CAF50"
            />
          </View>
        ...
        <View style={styles.cardContent}>
          ...
          {branch?.address && (
            <View style={styles.infoRow}>
              <Icon name="map-marker-radius" size={18} color="#757575" />
  ...
  if (isLoading) {
    return (
      <View style={styles.container}>
        <Header title="Lịch làm việc của tôi" showBack={true} />
